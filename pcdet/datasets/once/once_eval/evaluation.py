...
def get_evaluation_results(gt_annos, pred_annos, classes,
    ...
    # it means joint these three classes into one class: Vehicle, just for lidarcs
    ...
    # for lidarcs dataset, change the keys of pred_annos to match the keys of gt_annos
    # for pred_anno in pred_annos:
    #     pred_anno['boxes_3d'] = pred_anno['boxes_lidar']
    #     pred_anno.pop('boxes_lidar')
    # for gt_anno in gt_annos:
    #     gt_anno['boxes_3d'] = gt_anno['gt_boxes_lidar']
    #     gt_anno.pop('gt_boxes_lidar')
    
    # if 'boxes_lidar' in pred_annos[0].keys():
    #     for pred_anno in pred_annos:
    #         pred_anno['boxes_3d'] = pred_anno['boxes_lidar']
    #         pred_anno.pop('boxes_lidar')
    # if 'boxes_lidar' in gt_annos[0].keys():
    #     for gt_anno in gt_annos:
    #         gt_anno['boxes_3d'] = gt_anno['gt_boxes_lidar']
    #         gt_anno.pop('gt_boxes_lidar')
    ...
    # for xmu dataset, change the keys of pred_annos to match the keys of gt_annos
    if 'boxes_lidar' in pred_annos[0].keys():
        for pred_anno in pred_annos:
            pred_anno['boxes_3d'] = pred_anno['boxes_lidar']
            pred_anno.pop('boxes_lidar')
    if 'boxes_7DoF' in gt_annos[0].keys():
        for gt_anno in gt_annos:
            gt_anno['boxes_3d'] = gt_anno['boxes_7DoF']
            gt_anno.pop('boxes_7DoF')
    ...
    if difficulty_mode == 'Distance':
        ...
        num_difficulties = 4
        # difficulty_types = ['overall', '0-50m', '50-100m', '100m-inf'] # for xmu
        difficulty_types = ['overall', '0-30m', '30-50m', '50m-inf'] # for xmu
        # difficulty_types = ['0-70m', '0-30m', '30-50m', '50m-70m'] # for lidarcs dataset
    else:
