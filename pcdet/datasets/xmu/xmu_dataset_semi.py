...
class XMechanismUnmanned(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, sensor=None):
        ...
        # self.split_file = os.path.join(self.root_path, 'ImageSets', self.split + '_top_10_percent.txt')
    ...
    def get_lidar(self, idx, sensor, num_features=4):
        ...
        if to_ego:
            ...
            calib = self.get_calib(idx=idx, sensor=sensor)
    ...
    def set_split(self, split):
        ...
        self.split = split
        # split_dir = os.path.join(self.root_path, 'ImageSets', self.split+'_top_10_percent.txt')
    ...
    def create_groundtruth_database(self, info_path, used_classes=None, split='train'):
        ...
        for sensor in sensors:
            database_save_path = Path(self.root_path) / ('top_10_percent_gt_database_%s' % sensor if split == 'train' else 'top_10_percent_gt_database_%s_%s' % (self.sensor, split))
            db_info_save_path = Path(self.root_path) / ('top_10_percent_gt_database_info_%s.pkl' % sensor if split == 'train' else 'top_10_percent_gt_database_info_%s_%s.pkl' % (self.sensor, split))
...
def create_xmu_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    for sensor in sensors:
        ...
        train_filename = os.path.join(save_path, 'top_10_percent_xmu_infos_%s.pkl'%train_split)
